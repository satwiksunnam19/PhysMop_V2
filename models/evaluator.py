...
class evaluator(nn.Module):
    ...
    def forward_kinematics(self,pose,shape,gender_id,process_size_test,vertices=False,joints_smpl=False):
        ...
        output_smpl_m=self.smplh_m.forward(betas=shape[gender_id==0],rotmat=rotmat[gender_id==0])
        output_smpl_f=self.smplh_f.forward(betas=shape[gender_id==1],rotmat=rotmat[gender_id==1])
        ...
        if vertices:
            ...
            vertices[gender_id==0]=output_smpl_m.vertices
            vertices[gender_id==1]=output_smpl_f.vertices
            vertices[gender_id==2]=output_smpl.vertices
        else:
            vertices=None
        
        joints=torch.zeros([process_size_test,17,3]).float().to(self.device)
        joints[gender_id==0]=output_smpl_m.joints[:,:17]
        joints[gender_id==1]=output_smpl_f.joints[:,:17]
        joints[gender_id==2]=output_smpl.joints[:,:17]

        if joints_smpl==True:
            joints_smpl=torch.zeros([process_size_test,24,3]).float().to(self.device)
            joints_smpl[gender_id==0]=output_smpl_m.joints_smpl[:,:24]
            joints_smpl[gender_id==1]=output_smpl_f.joints_smpl[:,:24]
    ...
    def forward(self,data_test,input_type="numpy"):
        ...
        pred_q_data,pred_q_physics_gt,pred_q_physics_pred,pred_q_fusion,fusion_weight,pred_jacutation,pred_c,pred_jacutation_fusion,pred_c_fusion=model_output
        ...
        if config.test_mode == 'H36M':
            ...
            pred_joints_smpl_physics_gt[:,[0,1,4]]=0 
        ...
        return (gt_q.detach().cpu().numpy(), pred_q_physics_pred.detach().cpu().numpy(), pred_q_data.detach().cpu().numpy(), pred_q_fusion.detach().cpu().numpy()), (error_test_data, error_test_physics_gt, error_test_fusion, accel_data, accel_physics_gt, accel_fusion, gt_vertices, pred_vertices_fusion, fusion_weight.detach().cpu().numpy(),pred_jacutation,pred_c,pred_jacutation_fusion,pred_c_fusion)
